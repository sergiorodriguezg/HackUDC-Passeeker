...
let state = {
  ...
  sessionToken: null,
  vaultKey: null,       // Buffer(32) derivado de master
  lastActivity: 0,

  lockMs: 5 * 60 * 1000, // 5 min
  lockTimer: null,

  extTokens: new Set(),      // tokens válidos para la extensión (persistibles)
  pairCode: null,            // código temporal 6 dígitos
  pairExpiresAt: 0,          // timestamp caducidad
};
...
function loadExtTokens(dataDir) {
  ...
  try {
    ...
    // si está corrupto, empezamos vacío
    state.extTokens = new Set();
  }
}
...
export function startPairing(dataDir) {
  if (!isUnlocked()) throw new Error("Locked");

  // 6 dígitos, válido 60s
  const code = String(Math.floor(100000 + Math.random() * 900000));
  state.pairCode = code;
  state.pairExpiresAt = Date.now() + 60 * 1000;

  // opcional: guardamos última actividad para que no autolock justo ahora
  touch();
...
export function requireExtension(dataDir) {
  return (req, res, next) => {
    ...
    // App debe estar desbloqueada SIEMPRE
    if (!isUnlocked()) {
      return res.status(401).json({ ok: false, error: "Unauthorized (locked)" });
    }
